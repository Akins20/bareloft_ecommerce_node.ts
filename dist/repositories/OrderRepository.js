...
class OrderRepository extends BaseRepository_1.BaseRepository {
    ...
    async findByOrderNumber(orderNumber) {
        try {
            const order = await this.findFirst({ orderNumber }, {
                items: {
                    include: {
                        product: {
                            include: {
     ...
     */
    async getOrdersWithFilters(queryParams) {
        try {
            const { page = 1, limit = 20, status, paymentStatus, startDate, endDate, search, sortBy = "createdAt", sortOrder = "desc", } = queryParams;
            // Build where clause
            const where = {};
            // Status filter
            if (status) {
     ...
     * Get recent orders
     */
    async getRecentOrders(limit = 10, status) {
        try {
            const where = {};
            if (status) {
                where.status = status;
    ...
    async getOrderAnalytics(startDate, endDate) {
        try {
            ...
            const [totalOrders, totalRevenue, ordersByStatus, topProducts, topCategories, topCustomers, revenueByMonth,] = await Promise.all([
                this.count(where),
