...
class MetricsService extends BaseService_1.BaseService {
     ...
     */
    async recordMetric(name, value, tags = {}, timestamp = new Date()) {
        try {
    ...
    async getSystemMetrics() {
        try {
            ...
            }, { ttl: types_1.CONSTANTS.CACHE_TTL.SHORT });
        }
        catch (error) {
        ...
        try {
            ...
            return await this.cache.getOrSet(cacheKey, async () => {
                ...
                const totalInventoryValue = inventory.reduce((sum, item) => {
                    return sum + (item.quantity || 0) * Number(item.averageCost || 0);
                }, 0);
                const lowStockItems = inventory.filter((i) => (i.quantity || 0) <= (i.lowStockThreshold || 10));
                const outOfStockItems = inventory.filter((i) => (i.quantity || 0) <= 0);
                return {
                    revenue: {
                        total: totalRevenue,
                    ...
                    customers: {
                        total: users.length,
                        active: users.filter((u) => u.lastLoginAt &&
                            u.lastLoginAt >
            ...
            }, { ttl: types_1.CONSTANTS.CACHE_TTL.MEDIUM });
        }
        catch (error) {
    ...
    async getPerformanceMetrics() {
        try {
            ...
            return await this.cache.getOrSet(cacheKey, async () => {
                ...
                return {
                    api: {
                        requestsPerSecond: (await this.getMetricValue("api.requests_per_second")) || 10,
                        averageResponseTime: (await this.getMetricValue("api.response_time_avg")) || 150,
                        p95ResponseTime: (await this.getMetricValue("api.response_time_p95")) || 300,
                        errorRate: (await this.getMetricValue("api.error_rate")) || 0.5,
                        slowestEndpoints: await this.getSlowestEndpoints(),
                    },
                    database: {
                        connectionsActive: (await this.getMetricValue("db.connections_active")) || 5,
                        connectionsIdle: (await this.getMetricValue("db.connections_idle")) || 10,
                        slowQueries: (await this.getMetricValue("db.slow_queries")) || 0,
                        cacheHitRate: (await this.getMetricValue("db.cache_hit_rate")) || 85,
                        indexUsage: (await this.getMetricValue("db.index_usage")) || 95,
                    },
                    server: {
                        cpuUsage: (await this.getMetricValue("server.cpu_usage")) || 25,
                        memoryUsage: (await this.getMetricValue("server.memory_usage")) || 60,
                        diskUsage: (await this.getMetricValue("server.disk_usage")) || 45,
                        networkIO: (await this.getMetricValue("server.network_io")) || 1024,
     ...
     */
    async recordAPIRequest(endpoint, method, statusCode, responseTime, userId) {
        const tags = {
            endpoint,
            method,
        ...
        if (userId) {
            tags.user_id = userId; // Cast to avoid type error
        }
        await Promise.all([
     ...
     */
    async recordBusinessEvent(event, value = 1, properties = {}) {
        const tags = Object.entries(properties).reduce((acc, [key, val]) => {
            acc[key] = String(val);
            return acc;
        }, {});
        ...
        switch (event) {
            case "order_created":
                await this.cache.increment(`metrics:business:orders:${today}`, 1, 86400);
                if (properties.amount) {
                    await this.cache.increment(`metrics:business:revenue:${today}`, Math.round(properties.amount * 100), // Store in kobo
                    86400);
                }
                break;
            case "user_signup":
                await this.cache.increment(`metrics:business:signups:${today}`, 1, 86400);
                break;
            case "product_view":
                await this.cache.increment(`metrics:business:product_views:${today}`, 1, 86400);
                break;
        }
    }
    /**
     ...
     * Get metric history
     */
    async getMetricHistory(metricName, tags = {}, limit = 100) {
        try {
    ...
    async getHealthStatus() {
        try {
            ...
            try {
                await models_1.UserModel.findFirst();
                checks.database = {
                    status: "pass",
    ...
    async getMetricValue(metricName) {
        return await this.cache.get(`current:${metricName}`);
    }
    ...
    async getBusinessMetrics() {
        ...
        const [activeUsers, dailyOrders, dailyRevenue] = await Promise.all([
            this.cache.get(`metrics:business:active_users:${today}`) || 0,
            this.cache.get(`metrics:business:orders:${today}`) || 0,
            this.cache.get(`metrics:business:revenue:${today}`) || 0,
        ]);
        return {
            activeUsers,
            dailyOrders,
    ...
    async getTechnicalMetrics() {
        return {
            ...
            cacheHitRate: 85,
            apiCalls: (await this.cache.get(`metrics:api:requests:${new Date().toISOString().split("T")[0]}`)) || 0,
            backgroundJobs: 0,
            storageUsed: 45,
        };
    }
