...
class ReportingService extends BaseService_1.BaseService {
     ...
     */
    async generateSalesSummaryReport(startDate, endDate, format = 'excel') {
        try {
     ...
     */
    async generateProductPerformanceReport(startDate, endDate, format = 'excel') {
        try {
     ...
     */
    async generateCustomReport(type, startDate, endDate, filters = {}, format = 'excel') {
        try {
            switch (type) {
                case 'sales_summary':
                    return this.generateSalesSummaryReport(startDate, endDate, format);
                case 'product_performance':
                    return this.generateProductPerformanceReport(startDate, endDate, format);
    ...
    async getSalesSummaryData(startDate, endDate) {
        const [orders, previousPeriodOrders, orderItems] = await Promise.all([
            ...
            models_1.OrderItemModel.findMany({
                where: {
                    order: {
                        createdAt: { gte: startDate, lte: endDate },
                        status: { not: 'CANCELLED' },
    ...
    async getProductPerformanceData(startDate, endDate) {
        ...
        const productPerformance = products.map(product => {
            ...
            else if (sales.revenue > 5000)
                performance = 'good';
            else if (sales.revenue > 1000)
                performance = 'average';
            return {
                id: product.id,
                name: product.name,
    ...
    async generateReportAsync(reportId, request) {
        try {
            ...
            const buffer = await this.generateCustomReport(request.type, request.startDate, request.endDate, request.filters, request.format);
