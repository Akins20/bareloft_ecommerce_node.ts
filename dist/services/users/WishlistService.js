...
class WishlistService extends BaseService_1.BaseService {
    ...
    constructor(userRepository, productRepository) {
        super();
        this.userRepository = userRepository || new UserRepository_1.UserRepository(connection_1.prisma);
        this.productRepository = productRepository || new ProductRepository_1.ProductRepository(connection_1.prisma);
    }
    /**
     ...
     */
    async moveToCart(userId, productId, quantity = 1, removeFromWishlist = true) {
        try {
            // Check if product exists in wishlist
            const existsInWishlist = await this.isProductInWishlist(userId, productId);
            if (!existsInWishlist) {
     ...
     */
    async moveMultipleToCart(userId, items, removeFromWishlist = true) {
        try {
            let successCount = 0;
            let failureCount = 0;
            const failures = [];
            for (const item of items) {
                try {
                    const result = await this.moveToCart(userId, item.productId, item.quantity || 1, removeFromWishlist);
                    if (result.success) {
                        successCount++;
                    }
                    else {
     ...
     */
    async generateShareableLink(userId, isPublic = true, expiresInDays = 30) {
        try {
     ...
     */
    async getSharedWishlist(shareToken, options) {
        try {
     ...
     */
    async getBackInStockItems(userId) {
        try {
