...
exports.generateAuditSummary = exports.securityEventLogger = exports.performanceAuditLogger = exports.dataAccessAuditLogger = exports.authenticationAuditLogger = exports.adminAuditLogger = exports.financialAuditLogger = exports.auditLogger = void 0;
...
const detectAuditOperation = (req) => {
    ...
    if (path.includes("/inventory")) {
        return {
            ...
            metadata: {
                ...
                changeAmount: (req.body?.quantity || 0) - (req.body?.previousQuantity || 0),
    ...
    if (path.includes("/wishlist")) {
        return {
            ...
            metadata: {
                ...
                action: method === "POST" ? "add" : method === "DELETE" ? "remove" : "view",
                productName: req.body?.productName,
            },
        };
    }
...
const generateUserFingerprint = (req) => {
...
const detectStateFromIP = (ip) => {
    if (ip.startsWith("41."))
        return "Lagos";
    if (ip.startsWith("196."))
        return "FCT";
    return null;
};
// 🇳🇬 Check if IP is from Nigeria
const isNigerianIP = (ip) => {
    const nigerianRanges = ["41.", "196.", "105.", "154."];
    return nigerianRanges.some((range) => ip.startsWith(range));
};
// 🛡️ Calculate security risk level
const calculateRiskLevel = (req, auditEvent) => {
    let riskScore = 0;
    if (!req.user)
        riskScore += 1;
    if (["payment", "admin", "auth"].some((sensitive) => auditEvent.action.includes(sensitive))) {
        riskScore += 2;
    }
...
const detectSecurityThreats = (req, auditEvent) => {
    const threats = [];
    ...
    if (requestString.includes("<script") ||
        requestString.includes("javascript:")) {
        threats.push("potential_xss");
    }
    const userAgent = req.get("User-Agent") || "";
    if (/bot|crawler|scanner|hack/i.test(userAgent) &&
...
const checkPersonalDataInvolvement = (auditEvent) => {
...
const isConsentRequired = (auditEvent) => {
    return (auditEvent.action === "auth_signup" ||
        auditEvent.action.includes("marketing") ||
        auditEvent.action.includes("newsletter"));
};
// 📅 Get data retention period
const getRetentionPeriod = (auditEvent) => {
...
const categorizeAuditEvent = (auditEvent) => {
    if (auditEvent.action.includes("payment"))
        return "financial";
    if (auditEvent.action.includes("admin"))
        return "administrative";
    if (auditEvent.action.includes("auth"))
        return "authentication";
    if (auditEvent.action.includes("user"))
        return "user_management";
    if (auditEvent.action.includes("product"))
        return "inventory";
    return "general";
};
// ⚖️ Get audit severity
const getAuditSeverity = (auditEvent, statusCode) => {
    if (statusCode >= 500)
        return "critical";
    if (auditEvent.action.includes("payment") ||
...
const generateAuditTags = (auditEvent, req, res) => {
    const tags = [];
...
const shouldNotifyCompliance = (auditEvent, statusCode) => {
    return (auditEvent.action.includes("payment") ||
        auditEvent.action.includes("admin") ||
        (statusCode >= 500 && auditEvent.action.includes("user")));
};
// 📧 Send compliance notification
const sendComplianceNotification = (auditLog) => {
    winston_1.logger.info("COMPLIANCE_NOTIFICATION", {
...
const triggerSecurityAlert = (auditLog) => {
    winston_1.logger.error("SECURITY_ALERT", {
        alert: true,
        severity: "high",
    ...
    res.end = function (chunk, ...args) {
        ...
        const auditLog = {
            ...
            resource: {
                ...
                name: auditEvent.metadata?.productName ||
            ...
            // Correlation IDs
            correlation: {
                traceId: req.get("X-Trace-ID") || req.id,
...
const financialAuditLogger = (req, res, next) => {
    ...
    res.end = function (chunk, ...args) {
        const financialLog = {
            ...
            compliance: {
                ...
                riskAssessment: req.body?.amount > 500000
                    ? "high"
...
exports.financialAuditLogger = financialAuditLogger;
// 👑 Admin audit logger
    ...
    res.end = function (chunk, ...args) {
        const adminLog = {
            ...
            businessImpact: {
                ...
                severity: getBusinessImpactSeverity(req.path, req.method, res.statusCode),
                affectedUsers: req.body?.affectedUsers || "unknown",
                revenueImpact: req.body?.amount ? `₦${req.body.amount}` : "none",
            },
            compliance: {
                adminActionLogged: true,
                supervisoryReviewRequired: req.body?.amount > 1000000 || req.path.includes("/users"),
...
const authenticationAuditLogger = (req, res, next) => {
    ...
    res.end = function (chunk, ...args) {
        ...
        const authLog = {
            ...
            security: {
                ...
                otpUsed: authAction === "verify-otp" || req.body?.otpCode ? true : false,
...
exports.authenticationAuditLogger = authenticationAuditLogger;
...
exports.dataAccessAuditLogger = dataAccessAuditLogger;
// 📈 Performance audit logger
const performanceAuditLogger = (threshold = 1000) => {
    return (req, res, next) => {
...
exports.performanceAuditLogger = performanceAuditLogger;
...
const securityEventLogger = (eventType, details) => {
    return (req, res, next) => {
        ...
        winston_1.logger.error("SECURITY_AUDIT", securityLog);
        next();
    };
};
exports.securityEventLogger = securityEventLogger;
// 🔧 Helper functions
const getBusinessImpactCategory = (path, method) => {
...
const getBusinessImpactSeverity = (path, method, statusCode) => {
    if (statusCode >= 500)
        return "critical";
    if (method === "DELETE")
        return "high";
    if (path.includes("/payment") || path.includes("/orders"))
        return "high";
...
const calculateAuthRiskLevel = (req, action, success) => {
    let risk = 0;
...
const detectAuthThreats = (req, action, success) => {
    const threats = [];
    if (!success && action === "login")
...
const isDataAccessOperation = (path) => {
    const dataEndpoints = [
        "/users/",
        "/orders/",
        "/profile/",
...
const getDataResourceType = (path) => {
    if (path.includes("/users/") || path.includes("/profile/"))
        return "user_data";
    if (path.includes("/orders/"))
        return "order_data";
...
const getDataScope = (path, query) => {
    if (query.limit && parseInt(query.limit) > 100)
        return "bulk";
    if (path.includes("/admin/"))
        return "administrative";
...
const getDataRetentionPeriod = (path) => {
    if (path.includes("/payment/") || path.includes("/orders/"))
        return "7_years";
    if (path.includes("/users/") || path.includes("/profile/"))
        return "7_years";
...
const generateAuditSummary = (timeframe = "daily") => {
    return (req, res, next) => {
...
exports.default = {
    ...
    financialAuditLogger: exports.financialAuditLogger,
    adminAuditLogger: exports.adminAuditLogger,
    authenticationAuditLogger: exports.authenticationAuditLogger,
    dataAccessAuditLogger: exports.dataAccessAuditLogger,
    performanceAuditLogger: exports.performanceAuditLogger,
    securityEventLogger: exports.securityEventLogger,
